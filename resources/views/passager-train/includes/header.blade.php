<div class="user-body">
    <div class="row">
        <div class="col-sm-6">
            <div class="p-20 text-center">
                <a data-toggle="pill" href="#new-merchandise" role="tab"
                   class="btn btn-danger waves-effect">Nova Mercadoria</a>
            </div>
        </div>
    </div>
    <ul class="page-list nav nav-tabs flex-column" id="pills-tab" role="tablist">
        <li class="nav-item mail-section">
            <a class="nav-link waves-effect d-block active" data-toggle="pill" href="#e-inbox" role="tab">
                <i class="icofont icofont-inbox"></i> Todas Mercadoria
                <span class="label label-primary float-right">6</span>
            </a>
        </li>
        <li class="nav-item mail-section">
            <a class="nav-link waves-effect d-block" href="#merchandises_type" data-toggle="pill" role="tab">
                <i class="fa fa-balance-scale"></i>Categoria
            </a>
        </li>
        <li class="nav-item mail-section">
            <a class="nav-link waves-effect d-block" data-toggle="pill" href="#statistics" role="tab">
                <i class="fa fa-area-chart"></i> Estatísticas
            </a>
        </li>
    </ul>

</div>
